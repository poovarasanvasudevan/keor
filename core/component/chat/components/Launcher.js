...
class Launcher extends Component {
  ...
  render() {
    ...
    const classList = [ css['sc-launcher'],css[(isOpen ? 'opened' : '')]];
    return (
      <div id="sc-launcher">
        <div className={classList.join(' ')} onClick={this.handleClick.bind(this)}>
          <MessageCount count={this.props.newMessagesCount} isOpen={isOpen} />
          <img className={css["sc-open-icon"]} src={launcherIconActive} />
          <img className={css["sc-closed-icon"]} src={launcherIcon} />
        </div>
        <ChatWindow
...
const MessageCount = (props) => {
  ...
  return (
    <div className={css["sc-new-messages-count"]}>
      {props.count}
    </div>
  )
}
